...
def enable_vit_wrapper(vla_cfg):
    enable_vit_wrapper = False
    enable_vit_wrapper = enable_vit_wrapper or vla_cfg.featurizer_cfg["use_reg"] or vla_cfg.fused_featurizer_cfg["use_reg"]
    enable_vit_wrapper = enable_vit_wrapper or vla_cfg.featurizer_cfg["use_film"] or vla_cfg.fused_featurizer_cfg["use_film"]
    enable_vit_wrapper = enable_vit_wrapper or vla_cfg.featurizer_cfg["use_mm_sampler"] or vla_cfg.fused_featurizer_cfg["use_mm_sampler"]
    return enable_vit_wrapper
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    if enable_vit_wrapper(vla.config):
...
def _apply_film_to_vla(vla: torch.nn.Module, cfg: Any) -> torch.nn.Module:
    ...
    lora_config = LoraConfig(
        ...
        lora_dropout=0.0,
        target_modules=find_all_linear_names(vla, excluded_names=excluded_names),
        init_lora_weights="gaussian",
    )
    vla = get_peft_model(vla, lora_config)

    # Create and apply FiLMed vision backbone
    # vla_cfg = AutoConfig.from_pretrained(cfg.pretrained_checkpoint)
    new_vision_backbone = PrismaticHybridCompressionVisionBackbone(
...
def get_action_head(cfg: Any, llm_dim: int, num_action_tokens: int) -> Union[L1RegressionActionHead, DiffusionActionHead]:
    """
    ...
    if cfg.use_l1_regression:
        action_head = L1RegressionActionHead(input_dim=llm_dim, hidden_dim=llm_dim, action_dim=ACTION_DIM, num_action_tokens=num_action_tokens)
    elif cfg.use_diffusion:
        action_head = DiffusionActionHead(
            input_dim=llm_dim, hidden_dim=llm_dim, action_dim=ACTION_DIM, num_diffusion_steps=cfg.num_diffusion_steps, num_action_tokens=num_action_tokens
        )
    else:
...
def get_vla_action(
    ...
    with torch.inference_mode():
        ...
        if action_head is None:
            ...
            action, _ = vla.predict_action(**inputs, unnorm_key=cfg.unnorm_key, do_sample=False, instructions=prompt)
        else:
