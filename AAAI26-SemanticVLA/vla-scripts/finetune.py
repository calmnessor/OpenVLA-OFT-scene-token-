...
from prismatic.extern.hf.configuration_prismatic import OpenVLAConfig, get_default_featurizer_config
from prismatic.extern.hf.modeling_prismatic import OpenVLAForActionPrediction
from prismatic.extern.hf.processing_prismatic import PrismaticImageProcessor, PrismaticProcessor
from prismatic.models.action_heads import DiffusionActionHead, L1RegressionActionHead
from prismatic.models.backbones.llm.prompting import PurePromptBuilder
from prismatic.models.film_vit_wrapper import FiLMedPrismaticVisionBackbone
from prismatic.models.vit_wrapper import PrismaticHybridCompressionVisionBackbone
...
class FinetuneConfig:
    ...
    # New Model Args
    vision_aggregate_type: str = 'concat'
    num_action_tokens: int = -1
    action_token_type: str = 'empty'
    enable_cross_vit_interact: bool = False
    cross_vit_target_pairs: str = None
    cross_vit_type: str = 'default'
    featurizer_cfg: dict = field(default_factory=get_default_featurizer_config)
    fused_featurizer_cfg: dict = field(default_factory=get_default_featurizer_config)
...
def print_trainable_parameters(model, is_main_process=True):
    total_param = 0
    trainable_param = 0
...
def run_forward_pass(
    ...
    TOTAL_ACTION_TOKENS = vla.module.TOTAL_ACTION_TOKENS
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        actions_hidden_states = (
            ...
            .reshape(batch_size, TOTAL_ACTION_TOKENS, -1)
            .to(torch.bfloat16)
        )  # (B, act_chunk_len, D)
    ...
    for t in action_head.module.noise_scheduler.timesteps:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            actions_hidden_states = text_hidden_states[current_action_mask | next_actions_mask].reshape(
                batch_size, TOTAL_ACTION_TOKENS, -1
            )  # (B, act_chunk_len, D)
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        if enable_vit_wrapper(vla_config):
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.num_action_tokens != -1:
        assert cfg.use_l1_regression or cfg.use_diffusion, "Discrete decoding cannot be used when action tokens merging is used"
    ...
    vla_cfg.num_action_tokens = cfg.num_action_tokens
    vla_cfg.action_token_type = cfg.action_token_type

    vla_cfg.enable_cross_vit_interact = cfg.enable_cross_vit_interact
    vla_cfg.cross_vit_target_pairs = ast.literal_eval(cfg.cross_vit_target_pairs) if cfg.cross_vit_target_pairs is not None else None
    vla_cfg.cross_vit_type = cfg.cross_vit_type

    vla_cfg.featurizer_cfg = cfg.featurizer_cfg
    vla_cfg.fused_featurizer_cfg = cfg.fused_featurizer_cfg
    ...
    if cfg.use_lora:
        excluded_names = [
            'featurizer_proj', 'fused_featurizer_proj',
            'moe_router', 'router', 'sampler_proj', 'projector',
        ]

        if vla_cfg.featurizer_cfg.get('freeze', False):
            excluded_names.append('vision_backbone.featurizer')

        if vla_cfg.fused_featurizer_cfg.get('freeze', False):
            excluded_names.append('vision_backbone.fused_featurizer')

        lora_config = LoraConfig(
            r=cfg.lora_rank,
            lora_alpha=cfg.lora_alpha,
            lora_dropout=cfg.lora_dropout,
            target_modules=find_all_linear_names(vla, excluded_names=excluded_names),  # NEW
            init_lora_weights="gaussian",
        )
        vla = get_peft_model(vla, lora_config)

        for n, p in vla.model.projector.named_parameters():
            p.requires_grad = True

        if hasattr(vla.model, 'moe_router'):    # NEW
            print("Unfreezing MoE components ...")
            for n, p in vla.model.moe_router.named_parameters():
                p.requires_grad = True
        ...
        if hasattr(vla.model, 'featurizer_sampler_proj'):    # NEW
            print("Unfreezing `featurizer_sampler_proj` components ...")
            for n, p in vla.model.featurizer_sampler_proj.named_parameters():
                p.requires_grad = True

        if hasattr(vla.model, 'fused_featurizer_sampler_proj'):    # NEW
            print("Unfreezing `fused_featurizer_sampler_proj` components ...")
            for n, p in vla.model.fused_featurizer_sampler_proj.named_parameters():
                p.requires_grad = True

        # if hasattr(vla.model, 'text_router'):    # NEW
        #     print("Unfreezing TextGuidedSampler components ...")
        #     for n, p in vla.model.text_router.named_parameters():
        #         p.requires_grad = True

        vla.print_trainable_parameters()

    # FiLM setup
    if enable_vit_wrapper(vla_cfg):
        count_parameters(vla.vision_backbone, "vla.vision_backbone (original)")
        # Wrap vision backbone
        ...
        vla.model.vision_backbone = PrismaticHybridCompressionVisionBackbone(
        ...
        if vla_cfg.featurizer_cfg.get('freeze', False):
            for n, p in vla.model.vision_backbone.vision_backbone.featurizer.named_parameters():
                p.requires_grad = False

        if vla_cfg.fused_featurizer_cfg.get('freeze', False):
            for n, p in vla.model.vision_backbone.vision_backbone.fused_featurizer.named_parameters():
                p.requires_grad = False

        # Freeze the text encoder
        for n, p in vla.model.vision_backbone.named_parameters():
            if 'mm_sampler' in n and 'text_encoder' in n:
                p.requires_grad = False
    ...
    if cfg.use_l1_regression:
        action_head = init_module(
            ...
            cfg,
            device_id,
            {"input_dim": vla.module.llm_dim, "hidden_dim": vla.module.llm_dim, "action_dim": ACTION_DIM, "num_action_tokens": cfg.num_action_tokens},
            to_bf16=True,
        )
    ...
    # Start training
    print_trainable_parameters(vla, is_main_process=distributed_state.is_main_process)
