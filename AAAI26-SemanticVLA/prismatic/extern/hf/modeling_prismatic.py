...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    def __init__(self, config: PrismaticConfig) -> None:
        ...
        # # ========= ablation =========
        # from prismatic.models.slime import TextGuidedSampler
        # self.text_router = TextGuidedSampler(config)
        # self.vision_backbone.get_num_patches = lambda :36
        # # ============================
    ...
    def _aggregate_patch_features(self, patch_features, language_embeddings=None):
        ...
        if self.config.vision_aggregate_type == 'moe':
            assert patch_features[0].shape[1] == patch_features[1].shape[1], \
                f"Input embeddings should have same token count, but received {[x.shape for x in patch_features]}"
            ...
            image_embeds = self.moe_router([patches_llm_proj, patches_fused_llm_proj], average_language_embedding)
    ...
    def _process_vision_features(self, pixel_values, language_embeddings=None, instructions=None):
        ...
        if isinstance(self.vision_backbone, PrismaticVisionBackbone):  # original class
            patch_features = self.vision_backbone(pixel_values)  # (bsz, 256 * num_images, D)
        else:  # wrapper class that receives language_embeddings as input
            patch_features = self.vision_backbone(pixel_values, language_embeddings, instructions)  # (bsz, 256 * num_images, D)
    ...
    def forward(
        ...
        instructions: Optional[List[str]|torch.LongTensor] = None,  # NEW
        proprio=None,
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def __init__(self, config: OpenVLAConfig) -> None:
        ...
        if self.config.num_action_tokens == -1:
            ...
            self.TOTAL_ACTION_TOKENS = self.config.num_action_tokens * NUM_ACTIONS_CHUNK  # NEW
