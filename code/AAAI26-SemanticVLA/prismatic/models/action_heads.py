...
class L1RegressionActionHead(nn.Module):
    ...
    def __init__(
        ...
        n_arm = 2 if ACTION_DIM == 14 else 1  # Bimanual or Unimanual, HARD CODE 14 as workaround
        self.model_trans = MLPResNet(
            num_blocks=2, input_dim=input_dim * num_action_tokens, hidden_dim=hidden_dim, output_dim=3 * n_arm
        )
        self.model_rot = MLPResNet(
            num_blocks=2, input_dim=input_dim * num_action_tokens, hidden_dim=hidden_dim, output_dim=3 * n_arm
        )
        self.model_grip = MLPResNet(
            num_blocks=2, input_dim=input_dim * num_action_tokens, hidden_dim=hidden_dim, output_dim=1 * n_arm
        )
    ...
    def predict_action(self, actions_hidden_states):
        ...
        trans = self.model_trans(rearranged_actions_hidden_states)
        rot = self.model_rot(rearranged_actions_hidden_states)
        grip = self.model_grip(rearranged_actions_hidden_states)

        if ACTION_DIM == 14:
            # Bimanual, HARD CODE 14 as workaround
            action = torch.cat([
                trans[..., :3], rot[..., :3], grip[..., :1],
                trans[..., 3:], rot[..., 3:], grip[..., 1:]
            ], dim=-1)
        else:
            action = torch.cat([trans, rot, grip], dim=-1)
...
#             self.num_action_tokens = ACTION_DIM
#
#         self.model = MLPResNet(
#             num_blocks=2, input_dim=input_dim*self.num_action_tokens, hidden_dim=hidden_dim, output_dim=action_dim
#         )
#
#     def predict_action(self, actions_hidden_states):
#         # actions_hidden_states: last hidden states of Transformer corresponding to action tokens in sequence
...
class DiffusionActionHead(nn.Module):
    ...
    def __init__(
        ...
        assert num_action_tokens == -1, "TODO: implement action token merge for diffusion head"
        # TODO: implement action token merge for diffusion head
