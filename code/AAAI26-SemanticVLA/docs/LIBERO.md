...
- **Setting `--center_crop True` is important** because we fine-tuned SemanticVLA with random crop augmentations.
...
- The results reported in our paper were obtained using **Python 3.10 and PyTorch 2.2.0**
  ...
  Note that results may vary slightly if you use a different GPU or environment.
...
- We recommend mixing Libero's four subsets for joint training instead of training them separately.
