...
class VGGTSceneExtractor:
    ...
    def extract_scene_tokens(self, images_512: torch.Tensor) -> torch.Tensor:
        predictions = self.model(images_512.to(self.device, dtype=torch.bfloat16))
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        if cfg.use_scene_tokens:
            torch.save(
                vla.module.scene_projector.state_dict(), checkpoint_dir / f"scene_projector--{checkpoint_name_suffix}"
            )
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # Q-Former 模式: N*16 VGGT registers → K compressed task-conditioned tokens
    vla.scene_projector = SceneProjector(
        scene_dim=2048,
        llm_dim=vla.llm_dim,
        num_queries=8,           # K compressed tokens
        num_layers=2,            # Q-Former layers
        num_heads=8,             # Attention heads
        qformer_dropout=0.1,
        film_bottleneck=256,
        use_qformer=True,        # True: full pipeline; False: baseline
    ).to(device_id, dtype=torch_dtype)
    ...
    if cfg.use_proprio:
        NUM_PATCHES += 1
    # For VGGT-Omega scene tokens, add scene token count
    # Q-Former 模式: K 个压缩 token；简单模式: N*16 个原始 token
    if cfg.use_scene_tokens:
        if vla.module.scene_projector.use_qformer:
            NUM_SCENE_TOKENS = vla.module.scene_projector.num_queries  # K compressed tokens
        else:
            NUM_SCENE_TOKENS = cfg.num_images_in_input * 16  # N*16 raw tokens
