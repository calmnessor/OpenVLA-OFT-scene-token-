...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def _process_scene_tokens(self, projected_patch_embeddings, scene_tokens):
        """Project VGGT-Omega scene tokens and append to vision patch embeddings.

        VGGT-Omega registers encode global 3D geometry from multi-view input,
        serving as a spatial prior for the VLA policy. They are projected from
        1024d to llm_dim, then concatenated with the projected patch embeddings.
        """
        if scene_tokens is not None:
            # scene_tokens: [B, N*16, 1024] -> project -> [B, N*16, llm_dim]
            scene_embeddings = self.scene_projector(scene_tokens)
            # Concat: vision patches + scene geometry tokens
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            # Add VGGT-Omega scene tokens (global 3D geometry prior)
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def __init__(self, config: OpenVLAConfig) -> None:
        ...
        # Scene projector for VGGT-Omega scene tokens (register tokens)
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        Args:
            ...
            scene_tokens: Optional VGGT-Omega scene tokens [B, N*16, 1024]
        ...
        # Add VGGT-Omega scene tokens if provided
        if scene_tokens is not None:
            projected_patch_embeddings = self._process_scene_tokens(
                projected_patch_embeddings, scene_tokens
            )
        ...
        # Calculate number of patches (including scene tokens, proprio token and/or diffusion timestep embedding)
        NUM_PATCHES = self.vision_backbone.get_num_patches() * self.vision_backbone.get_num_images_in_input()
        if scene_tokens is not None:
            NUM_PATCHES += scene_tokens.shape[1]  # N * 16
        if use_proprio:
            NUM_PATCHES += 1
        if use_diffusion:
            NUM_PATCHES += 1
