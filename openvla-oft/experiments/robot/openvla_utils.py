...
from prismatic.models.projectors import GeoRegisterPredictor, NoisyActionProjector, ProprioProjector
...
def _has_hf_model_weights(checkpoint_dir: str) -> bool:
    """Return whether a local checkpoint directory contains full HF model weights."""
    ...
    for filename in os.listdir(checkpoint_dir):
        if filename in weight_filenames:
            return True
...
def _get_unmerged_lora_base_checkpoint(pretrained_checkpoint: str) -> Optional[str]:
    """
    Detect checkpoints saved with merge_lora_during_training=False.

    Those directories contain lora_adapter/ plus action/proprio heads, but no full
    HF model shards. For evaluation we need to load the recorded base model first
    and then apply the LoRA adapter in memory.
    """
    if not os.path.isdir(pretrained_checkpoint):
        return None
    if _has_hf_model_weights(pretrained_checkpoint):
        return None

    adapter_config_path = os.path.join(pretrained_checkpoint, "lora_adapter", "adapter_config.json")
    ...
    if not base_checkpoint:
        raise ValueError(
            f"Found LoRA adapter in {pretrained_checkpoint}, but adapter_config.json does not contain "
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    # actually go into effect. Checkpoints saved with merge_lora_during_training=False
    # only contain a LoRA adapter; for those, load the adapter's recorded base model first.
    model_load_path = cfg.pretrained_checkpoint
    ...
    if not model_is_on_hf_hub(cfg.pretrained_checkpoint):
        ...
        base_checkpoint = _get_unmerged_lora_base_checkpoint(cfg.pretrained_checkpoint)
        if base_checkpoint is not None:
            model_load_path = base_checkpoint
            lora_adapter_dir = os.path.join(cfg.pretrained_checkpoint, "lora_adapter")
            print("Detected unmerged LoRA checkpoint.")
            print(f"  Base model: {model_load_path}")
            print(f"  LoRA adapter: {lora_adapter_dir}")
        else:
            # Update config.json and sync model files for full local checkpoints.
    ...
    if lora_adapter_dir is not None:
        ...
        print("Merged LoRA adapter into base model in memory.")
...
def get_geo_register_predictor(cfg: Any, llm_dim: int) -> GeoRegisterPredictor:
    """
    Load the lightweight geometry register predictor saved during geometry-distilled fine-tuning.

    VGGT-Omega is only a training-time teacher. At inference, this module predicts geometry
    register tokens directly from OpenVLA visual tokens.
    """
    geo_register_predictor = GeoRegisterPredictor(
        llm_dim=llm_dim,
        num_registers=getattr(cfg, "num_geo_registers", 16),
        num_heads=getattr(cfg, "geo_register_num_heads", 8),
        num_layers=getattr(cfg, "geo_register_num_layers", 2),
    ).to(DEVICE)
    geo_register_predictor = geo_register_predictor.to(torch.bfloat16).to(DEVICE)
    geo_register_predictor.eval()

    if model_is_on_hf_hub(cfg.pretrained_checkpoint):
        raise ValueError("Geometry-register checkpoints are only supported from local fine-tuned checkpoints.")

    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "geo_register_predictor")
    state_dict = load_component_state_dict(checkpoint_path)
    geo_register_predictor.load_state_dict(state_dict)
    return geo_register_predictor
...
def get_vla_action(
    ...
    geo_register_predictor: Optional[torch.nn.Module] = None,
